<script lang="ts" module>
	...
	const toastVariants = cva("pointer-events-auto box-border w-full rounded-lg px-4 py-3 text-xs shadow-lg", {
		defaultVariants: {
			variant: "default",
		},
		variants: {
	...
	export type ToastProps = NativeToastProps &
		Omit<ToastData, "duration" | "position"> & {
			class?: string;
		};
</script>
...
<script lang="ts">
	...
	let { class: className = "", dismissible, id, message, title, variant, ...restProps }: ToastProps = $props();
