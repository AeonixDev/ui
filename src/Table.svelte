<script lang="ts" module>
	...
	const getCellValue = (row: TableRow, column: string) => (row as Record<string, unknown>)[column];

	const getRowOption = <Row extends TableRow>(options: TableRowOptions<Row> | undefined, column: string) =>
	...
	export type TableRowSnippet<Row extends TableRow> = Snippet<[row: Row, key: Extract<keyof Row, string>]>;
					...
					<tr>
						{#each columns as column}
							<td>
								...
								{:else}
									{formatCellValue(getCellValue(row, column), getRowOption(rowOptions, column)?.type)}
								{/if}
							</td>
						{/each}
					</tr>
